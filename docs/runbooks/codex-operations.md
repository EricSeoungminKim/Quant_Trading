...
`PUBLICATION_HOLD`를 제거해 위 두 게시 일정을 복원했다. EC2의 활성 cron은
94개이며 그중 공개 게시 작업은 2개다. 22개 전략·537왕복의
...
09-14에는 별도 사용 한도를 가진 `gpt-5.3-codex-spark`로 실제 KR 리포트 생성과
축약 입력을 검증했다. 실행은 성공했으나 의미·출력 계약 검토를 통과하지 못해
운영 기본 모델은 Spark로 바꾸지 않았다. 실측과 적용 조건은
[Spark 리포트 검증](spark-report-evaluation.md)을 따른다. 전역 `CODEX_MODEL`은
보고서 외 Telegram 질의·JSON 작업에도 영향을 주므로 보고서 전환에 쓰지 않는다.
...
- 격리한 EC2 코드·데이터 사본으로 09-14 KR 리포트를 실제 생성했다: 589.6초,
  exit 0, HTML·엔진 JSON·후보 목록 생성. 사전 검증의 720초 예산 안에 완료됐고
...
- 같은 방식의 US 리포트도 472.2초에 exit 0으로 HTML·엔진 JSON·후보 목록을
  생성했다. 두 검증 모두 실제 API와 Codex를 사용했고 720초 이내였다.
  운영 systemd의 실제 제한은 발행 전 대기까지 포함한 `TimeoutStartSec=7200`이다.
...
- 로컬 전체 테스트의 기존 실패 14건은 수정 전 HEAD의 별도 사본에서도
  ...
  이후 테스트의 기준 시각·시간대·공급자를 명시해 14건을 수리했다. US 정적
  휴장표의 실제 누락 3개도 공식 일정으로 보완했다. 운영의 36시간 신선도
  기준은 유지한다. 전체 재검증은 7,188 통과/0 실패/10 skip/1 xfail이다.
- `6b837b2`를 EC2에 배포했다. 휴장표를 프로세스에 반영하기 위해 KR/US
  정규장 밖에서 거래 엔진을 재시작했고 정상 조립 로그를 확인했다.
  엔진 PID는 1979164, Telegram 브리지는 1979273이며 두 서비스와 웹 서버가
  active다. 앞선 배포에서 PID를 유지했다는 기록은 이 재시작 이전의 사실이다.
