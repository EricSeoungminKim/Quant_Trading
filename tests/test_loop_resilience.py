...
def test_universe_roll_has_a_boundary_before_us_open():
    ...
    assert d(8, 26) == "2026-08-11"
    # KR 동시호가(08:30) 직전 경계 (2026-08-17 전진: 08:57 → 08:27)
    assert d(8, 27) == "2026-08-11+0827"
    assert d(13, 59) == "2026-08-11+0827", "KR 오후 마감 리포트 흡수 전에는 경계가 지나지 않는다"
    # 13:55 own_brief.sh KR close → 13:58 데드라인 → 14:00 경계(서브프로젝트 T Task 2)
    assert d(14, 0) == "2026-08-11+1400"
    assert d(15, 30) == "2026-08-11+1400", "KR 세션 마감(15:30)까지도 14:00 버킷이 유지된다"
    # 21:50 US 발굴 직후 ~ 22:10 전: 아직 14:00 버킷 (아직 흡수 전)
    assert d(22, 9) == "2026-08-11+1400"
    # US 개장 직전 경계 — 여기서 새 US 종목이 흡수된다
    assert d(22, 10) == "2026-08-11+2210"
    # US 개장(22:30 EDT / 23:30 EST) 이후에도 같은 버킷 = 장중 스냅샷 안정
    assert d(22, 30) == "2026-08-11+2210"
    assert d(23, 30) == "2026-08-11+2210"
    assert d(23, 59) == "2026-08-11+2210"
...
def test_universe_roll_buckets_are_strictly_ordered_within_a_day():
    ...
    assert seen == [
        "2026-08-11", "2026-08-11+0827", "2026-08-11+1400", "2026-08-11+2210",
    ]
