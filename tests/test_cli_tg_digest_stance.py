"""`quant.apps.cli._tg_digest_stance_call`(2026-09-07, Codex CLI 주 레인 전환).

이전엔 `OPS_NARRATOR=openrouter`일 때만 스탠스 마이크로프롬프트가 동작했다
(owner 실측: 무료 레인이 한도 초과로 상시 실패). 기본값(미설정/`codex`)에서도
Codex CLI 1순위 + OpenRouter 폴백(`narrate.stance`)으로 동작하게 바꿨다 —
...
def test_default_codex_lane_builds_a_callable_when_binary_exists(monkeypatch):
    monkeypatch.delenv("OPS_NARRATOR", raising=False)
    monkeypatch.setenv("CODEX_BIN", __file__)  # 존재하는 파일이면 충분(실행하지 않는다)
...
def test_default_codex_lane_is_none_when_nothing_is_available(monkeypatch):
    monkeypatch.delenv("OPS_NARRATOR", raising=False)
...
def test_default_codex_lane_calls_stance_with_both_transports(monkeypatch):
    """`OPS_NARRATOR` 미설정이면 `narrate.stance`(codex 1순위 + openrouter
...
def test_openrouter_choice_stays_openrouter_only(monkeypatch):
    """명시적으로 `OPS_NARRATOR=openrouter`면 codex 를 시도하지 않는다 —
    ...
    monkeypatch.setattr(narrate_mod, "stance",
                        lambda *a, **k: (_ for _ in ()).throw(AssertionError("codex 경로를 타면 안 된다")))
