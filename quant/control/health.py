...
JOB_SCHEDULE: dict[str, tuple[str, int]] = {
    ...
    "ops-judge": ("weekday", 1),
    # 웨어하우스 적재(systemd 타이머) — 월~금 16:10 + 화~토 07:10. ops-judge와
    # 같은 이유로 같은 근사("weekday", 1영업일 허용)를 쓴다: 토요일 새벽 런이
    # "기대 밖"이 되는 대가로, 일요일/월요일 아침 점검이 주말 공백을 거짓 경보로
    # 잘못 읽지 않는다(2026-09-07 실측 — 일반 30시간 TTL로는 매주 일/월요일
    # "ingest: 최근 성공이 없다"가 거짓으로 났다).
    "ingest": ("weekday", 1),
}
