...
def _universe_roll_bucket(now: datetime | None = None) -> str:
    ...
    **14:00(KR 오후 마감 포지션 리포트 흡수)** + **22:10(US 개장 직전)**.
    ...
    - **14:00 (KR, 2026-08-17 서브프로젝트 T Task 2)**: 13:50 발행 오후 마감 포지션
      리포트(`{MARKET}_close_engine.json`)의 당일 단타 후보(EVENT_SCALP)가
      13:55 `own_brief.sh KR close` → 13:58 데드라인 → 이 경계로 흡수돼 14:00~15:30
      마감 구간에 매매 대상이 된다. 08:27 경계와 같은 목적(그 세션의 자동 편입이
      바로 반영)이지만 세션이 하나 더 생긴 것 — 08:30 동시호가 전용 경계가 15:30
      마감까지 커버하지 못하던 공백을 메운다.
