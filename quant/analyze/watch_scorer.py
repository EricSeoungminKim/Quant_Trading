...
**다음 감사를 위한 메모(2026-09-06 리포트 정확도 감사).** 이 채점기는 캔들
(OHLCV) 기반 3프로필뿐이라 `ai_score100`/`trending_score100`/`origin`(뉴스+
트렌딩 랭킹 동시 확인 여부)을 전혀 보지 않는다 — 프리퍼시티 게이트도 증거
점수도 이 필드들을 입력으로 받지 않는다. 그런데 그 감사는 origin="both"
(뉴스+트렌딩 동시 확인) 종목의 D+1 평균 수익률이 +278bp(적중 56.0%,
n=166)로 news-only(+28bp)·watch_join(+35bp)을 압도한다는 걸 확인했다
(`quant.analyze.symbol_score` docstring에 같은 근거를 인용해 반영해 뒀다).
이 파일은 그 신호를 아직 안 쓴다 — 다음 감사/개편 때 프리퍼시티 게이트가
아니라 증거 점수 쪽에(하드 게이트는 "근거 있음/없음"만 가르는 자리라
방향성 보너스와 성격이 다르다) `sector_daily_adjustment`류의 bounded 가감
후보로 검토할 것.
"""
