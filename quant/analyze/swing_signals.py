...
## 2026-09-06 정정 — 위 OOS 숫자는 더 이상 유효하지 않다

quant-backtest 의 KR 일봉 레이크에 날짜 밀림 결함(타임스탬프 앵커 혼재로 리더가 +1일)이 있었고,
수리 후 재실행(`results/swing_signals/RERUN_2026-09-06.md`)에서 단기반전 t 3.22→1.13(DSR 0.58→0.03),
거래량충격 t 2.95→0.86(DSR 0.67→0.04)으로 유의성이 사라졌다. 신호 계산 코드는 그대로 두되
(추천은 주문 없이 참고용), `manual_recs._KIND_EVIDENCE` 는 이 사실을 그대로 적는다. 재검증 통과
전에는 이 신호로 자본을 배정하지 않는다.
