...
# 2026-09-06 정정: 위 실측은 KR 일봉 레이크의 날짜 밀림 결함(변경기록 2026-09-06)이 있던 데이터에서
# 나온 것이다. 레이크 수리 후 재실행(quant-backtest results/swing_signals/RERUN_2026-09-06.md)에서
# 단기반전 t 3.22→1.13·DSR 0.58→0.03, 거래량충격 t 2.95→0.86·DSR 0.67→0.04 로 **OOS 유의성이
# 사라졌다.** 추천은 계속 내되(주문 없음, 소유자 참고용) 근거 문구는 사실대로 쓴다.
_KIND_EVIDENCE = {
    _STR_KIND: (
        "⚠️ 근거 재검증 결과 유의성 없음(2026-09-06 데이터 수리 후 OOS t=1.1, DSR 0.03; 수리 전 "
        "+17.5bp/t=3.2 는 결함의 산물) — 참고용, 표본 KR 대형주 walk-forward"
    ),
    _VSP_KIND: (
        "⚠️ 근거 재검증 결과 유의성 없음(2026-09-06 데이터 수리 후 OOS t=0.9, DSR 0.04; 수리 전 "
        "+51bp/t=2.95 는 결함의 산물) — 참고용, 표본 KR 대형주 walk-forward"
    ),
}
