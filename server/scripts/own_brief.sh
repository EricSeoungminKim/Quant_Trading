...
# -gt(초과)여야 한다: close 크론은 데드라인과 같은 분(14:52)에 뜨므로 -ge면
# 자기 실행 시각에 항상 걸려 편입이 100% 생략된다 — 2026-08-27 실사고(빌드는
# 14:28에 멀쩡히 끝났는데 CLOSE_BET 태그 0개). 편입 소요 실측 ~17초라
# 데드라인 분(分) 내 시작이면 다음 분의 유니버스 롤에 탑승한다.
